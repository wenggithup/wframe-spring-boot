...
public class MybatisPlusCodeGenerator {
    ...
    private static final String URL = "jdbc:mysql://192.168.0.85:11306/IM_USER?useUnicode=true&characterEncoding=utf8&";
    private static final String USERNAME = "root";
    private static final String PASSWORD = "rCaIMK*!Z#SWdEKo";
    ...
    public static void main(String[] args) {
        ...
        String projectModuleName = scanner("请输入项目模块名称");
        // 如果项目模块名为空
        if (StringUtils.isBlank(projectModuleName)) {
            ...
            //递归查找
            List<String> list = searchAllFileDirs(projectPath, new ArrayList<>());
            AtomicReference<String> path = new AtomicReference<>("");
            for (String t : list) {
                if (t.contains(outPutDir.toString())){
                    path.set(t);
                    break;
                }
            }
...
/*        if ("0".equals(scanner("是否有逻辑删除字段(0-否,1-是)"))) {
    ...
    public static List<String> searchAllFileDirs (String path,List<String> list){
        File file = new File(path);
        for (File listFile : file.listFiles()) {
            if (listFile.isDirectory()){
                list.add(file.getAbsolutePath());
                searchAllFileDirs(listFile.getAbsolutePath(),list);
            }
        }
        return list;
    }
